...
async function processOrder(shopify, step) {
  ...
  const { selectedCustomer, selectedProduct, selectedVariant } = await step.do(
    ...
    async () => {
      ...
      const productWithPrice = products.find(
        (edge) =>
          edge?.node?.variants?.edges?.[0]?.node?.price && parseFloat(edge.node.variants.edges[0].node.price) > 0
      );
    ...
    if (!orderResponse?.orderCreate?.order) {
      ...
      const errorMsg =
        userErrors && userErrors.length > 0
          ...
          : "Unknown error or unexpected response format.";
